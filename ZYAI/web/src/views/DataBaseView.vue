...
const embedModelOptions = computed(() => {
  // 增强健壮性，确保embed_model_names是有效的对象
  const modelNames = configStore.config?.embed_model_names || {};
  // 处理可能的空对象情况，返回一个默认的选项
  if (Object.keys(modelNames).length === 0) {
    return [{ label: '默认嵌入模型', value: 'default' }];
  }
  // 安全地遍历模型名称并创建选项
  return Object.keys(modelNames).map(key => ({
    label: `${key} (${modelNames[key]?.dimension || '未知维度'})`,
    value: key,
  }));
})
