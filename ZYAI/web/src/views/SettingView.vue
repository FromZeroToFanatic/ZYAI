...
const items = computed(() => {
  // 添加多层默认值处理，确保任何情况下都返回有效对象
  const configItems = (configStore.config || {})._config_items || {};
  return {
    embed_model: configItems.embed_model || { des: '嵌入模型', choices: [] },
    reranker: configItems.reranker || { des: '重排序模型', choices: [] }
  };
})
...
onMounted(() => {
  ...
  state.section = 'base' // 所有用户都可以访问基本设置
})
